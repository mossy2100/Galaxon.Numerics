...
public class Quantity
{
    ...
    internal const string RXS_UNIT_EXP = @"(?<unitExp>(-?\d+)?)";
    internal const string RXS_PREFIX_BASE = @"(?<prefixBase>[a-zΩ°""'″′]{1,6})";
    internal const string RXS_UNIT = $@"(?<unit>{RXS_PREFIX_BASE}{RXS_UNIT_EXP})";
    internal const string RXS_OP = @"(?<op>[\*⋅/])";
    internal const string RXS_OP_UNIT = $"({RXS_OP}{RXS_UNIT})";
    internal const string RXS_SIGN = "(?<sign>[-+]?)";
    internal const string RXS_DECIMAL = @"(?<amt>\d+(\.\d+)?)";
    internal const string RXS_CURRENCY_SYMBOL = $"(?<currencySymbol>[{BaseUnit.CURRENCY_SYMBOLS}])";
    internal const string RXS_CURRENCY_PREFIX = $"(?<currencyPrefix>[{UnitPrefix.CURRENCY_PREFIXES}]?)";
    internal const string RXS_MONEY =
        $@"({RXS_SIGN}{RXS_CURRENCY_SYMBOL}{RXS_DECIMAL}{RXS_CURRENCY_PREFIX}{RXS_OP_UNIT}*)";
    internal const string RXS_DOUBLE = @"(?<amt>([-+]?\d+(\.\d+)?(e[-+]?\d+)?)?)";
    internal const string RXS_COMPOUND_UNIT = $@"(?<compound>(?<op>/?){RXS_UNIT}{RXS_OP_UNIT}*)";
    internal const string RXS_PHYSICAL = $"{RXS_DOUBLE} ?{RXS_COMPOUND_UNIT}";
    ...
    public static Quantity Parse(string strQuantity)
    {
        ...
        Match match = Regex.Match(strQuantity, $"^{RXS_MONEY}$", RegexOptions.IgnoreCase);
        bool currencyMatch = match.Success;
        bool physicalMatch = false;
        if (!currencyMatch)
        {
            // Check for valid physical amount string.
            match = Regex.Match(strQuantity, $"^{RXS_PHYSICAL}$", RegexOptions.IgnoreCase);
    ...
    public string ToString(bool includeAmount, bool includeUnits, bool niceFormat = true)
    {
        ...
        for (int i = 0; i < Units.Count; i++)
        {
            ...
            else
            {
                ...
                if (BaseUnit.CURRENCY_SYMBOLS.Contains(strSymbol) && unit.Exponent == 1)
                {
